<?php
require"conn.php";
$result=mysql_query("select * from indexpage");
$indexpage=mysql_fetch_object($result);
?>
<!DOCTYPE html>
<head>
<title>::Home::</title>
<link rel="shortcut icon" href="admin/images/<?php echo $indexpage->fev ?>" />
<link rel="stylesheet" type="text/css" href="style.css" />
<script type="text/javascript" src="validation.js"></script>
</head>

<body>

<div class="container">
<a href="www.facebook.com/hoteldeluxe99">
<div class="share">&nbsp;&nbsp;
<ol id="share"><li>Facebook</li></ol>
</div></a>
<a href="www.twitter.com/hoteldeluxe99">
<div class="share1">&nbsp;&nbsp; 
<ol id="share"><li>Twitter</li></ol>
</div></a>

<a href="www.googleplus.com/hoteldeluxe">
<div class="share2">&nbsp;&nbsp; 
<ol id="share"><li>Google +</li></ol>
</div></a>

<a href="www.linkedin.com/hoteldeluxe99">
<div class="share3">&nbsp;&nbsp; 
<ol id="share"><li>Linked in</li></ol>
</div></a>

<a href="www.facebook.com/hoteldeluxe99">
<div class="share4">&nbsp;&nbsp; 
<ol id="share"><li>Pinterest</li></ol>
</div></a>

<a href="www.tumber.com/hoteldeluxe99">
<div class="share5">&nbsp;&nbsp; 
<ol id="share"><li>Tumbler</li></ol>
</div></a>



<div class="header">
<a href="index.php"><div class="logo"></div></a><div class="menu">
	<ul type="none">
    	<a href="index.php"><li>HOME</li></a>
        <a href="services.php"><li>SERVICES</li></a>
        <a href="accomodation.php"><li>ACCOMODATION</li></a>
        <a href="conference.php"><li>CONFERENCE</li></a>
      <a href="contactus.php"><li>CONTACT US.</li></a></ul>
</div>
</div>
<div class="slider"></div>
<div class="content"><b><?php echo $indexpage->content ?></b>
	<div class="con"><?php echo $indexpage->con ?></div>
    <br />
    <img width="60px" height="136px" src="admin/images/<?php echo $indexpage->arrowl ?>" />
    <img width="159px" height="136px" src="admin/images/<?php echo $indexpage->img1 ?>" />
    <img width="159px" height="136px" src="admin/images/<?php echo $indexpage->img2 ?>" />
    <img width="159px" height="136px" src="admin/images/<?php echo $indexpage->img3 ?>" />
    <img width="60px" height="136px" src="admin/images/<?php echo $indexpage->arrowr ?>" />
    
    </div>

<div class="backgroundd">
<div class="detail"><?php echo $indexpage->detail  ?></div>
<div class="room"><a href="standerdrooms.php?uname=standerd double room">
<img class="bor" src="admin/images/<?php echo $indexpage->sdrimg ?>" width="271px" height="92px" alt="standerd double room" /></a>
<b><?php echo $indexpage->roomtype1 ?></b>
<div class="roominfo"><?php echo $indexpage->roominfo ?></div>			
						<div class="img0">
                        		<img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
</div>
<div class="room"><a href="supperiorrooms.php?uname=supperior double room">
<img class="bor" src="admin/images/<?php echo $indexpage->sudrimg ?>" width="271px" height="92px" alt="supperior double room" /></a>
<b><?php echo $indexpage->roomtype2 ?></b><div class="roominfo"><?php echo $indexpage->roominfo ?></div> 								<div class="img0">
<img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
                                </div>
<div class="room"><a href="viproom.php?uname=vip room">
<img class="bor" src="admin/images/<?php echo $indexpage->vimg ?>" width="271px" height="92px" alt="vip room" /></a><b><?php echo $indexpage->roomtype3 ?></b><div class="roominfo"><?php echo $indexpage->roominfo ?>
</div>
 								<div class="img0">
                                <img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
                                </div>
<div class="room"><a href="standerdroom.php?uname=standerd single room">
<img  class="bor" src="admin/images/<?php echo $indexpage->ssrimg ?>" width="271px" height="92px" alt="standerd single room" /></a>
<b><?php echo $indexpage->roomtype4 ?></b><div class="roominfo"><?php echo $indexpage->roominfo ?></div> 								<div class="img0">
<img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
                                </div>
<div class="room"><a href="supperiorroom.php?uname=supperior single room">
<img class="bor" src="admin/images/<?php echo $indexpage->surimg ?>" width="271px" height="92px" alt="supperior single room" /></a>
<b><?php echo $indexpage->roomtype5 ?></b><div class="roominfo"><?php echo $indexpage->roominfo ?></div> 								<div class="img0">
<img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
                                </div>
<div class="room"><a href="familyroom.php?uname=family room">
<img class="bor" src="admin/images/<?php echo $indexpage->fimg ?>" width="271px" height="92px" alt="family room" /></a><b><?php echo $indexpage->roomtype6 ?></b><div class="roominfo"><?php echo $indexpage->roominfo ?></div> 								<div class="img0">
<img src="admin/images/<?php echo $indexpage->fac1 ?>" width="20px" height="15px" />
								<img src="admin/images/<?php echo $indexpage->fac2 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac3 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac4 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac5 ?>" width="20px" height="15px" />
                                <img src="admin/images/<?php echo $indexpage->fac6 ?>" width="20px" height="15px" />
                                </div>
                                </div>
</div>

<div class="pic"><img src="admin/images/<?php echo $indexpage->pic ?>" /></div>
<div class="yw"><div class="fnt"><b><?php echo $indexpage->fnt ?></b></div>
<div class="fntt"><?php echo $indexpage->fntt ?> </div> </div>
<div class="topfooter">

<div class="about">
<div class="txt"><?php echo $indexpage->txt ?>
<div class="txt1"><?php echo $indexpage->txt1 ?></div>
</div>
</div>

<div class="about">
<div class="txt"><?php echo $indexpage->txt2 ?>
<div class="txt1"><?php echo $indexpage->txt3 ?></div>
</div>
</div>
<div class="join">
<div class="txt4"><b><?php echo $indexpage->txt4 ?> </b></div>
<div class="txt1"><?php echo $indexpage->txt5 ?>
</div>
</div>
<div class="mamber">
<?php
require"conn.php";
$result=mysql_query("select * from custmer");
$custom=mysql_fetch_object($result);
?>
<div class="loginn" >
<a href="loginnnn.php"><input type="button" value="login" class="but0"></a>
</div>
<div class="loginn" >
<a href="admin/index.php"><input type="button"  value="Admin login" class="but"></a>
<br>
<!--social links-->
<div class="social">
<a href="www.fb.com/hoteldeluxe99"><img src="admin/images/fb.png" width="15px" height="15px"></a>
</div>
<div class="social">
<a href="www.twitter.com/hoteldeluxe"><img src="admin/images/tweet.png" width="15px" height="15px"></a>
</div>
<div class="social">
<a href="www.googleplus.com/hoteldeluxe99"><img src="admin/images/google+.png" width="15px" height="15px"></a>
</div>
<div class="social">
<a href="www.linkedin.com/hoteldeluxe99"><img src="admin/images/linked.png" width="15px" height="15px"></a>
</div>
<div class="social">
<a href="www.pinterest.com/hoteldeluxe99">
<img src="admin/images/Pinterest.png" width="15px" height="15px"></a>
</div>
<div class="social">
<a href="www.tumber.com/hoteldeluxe99">
<img src="admin/images/tumber.png" width="15px" height="15px"></a></div>





</div>

<!--<form name="myform1" action="bill.php" onSubmit="return validation1()" method="post">
<div class="input1">username<br /><input name="username" title="username" class="bor" type="text"  size="8" />
</div>
<div class="input1">password<br /><input title="password" name="Password" class="bor" type="password" size="8" /></div>
<div class="input1"><input class="bor1" name="login" type="submit" style="background-color:#cf822f" 
value=":Log in:"/>
</div>
</form>
-->
</div>
</div>
<div class="copyrightt"><a href="adminpage.php"><?php echo $indexpage->copyrightt ?></a></div>
</div>
</body>
</html>
